...
file_prefix = 'Sphere_hr4796_klipFM_3g_MaxiNoise'
...
if first_time == 1:
    ...
    theta_here = (np.log(74.213120),np.log(100.014140), 13.5211476,  0.99997934,  0.05089959 , 0.04771195 , 0.99948175 , -0.03397273 , np.cos(np.radians(76.7569)), 26.88106,-3.1004,1.49298, np.log(137.818))  
...
if __name__ == '__main__':
    ...
    # with contextlib.closing(MPIPool()) as pool:
        ...
        if new_backend ==1:
            ...
            w8 = np.random.uniform( theta_here[8]*0.99  ,   theta_here[8]*1.01  ,   size=(nwalkers))    #cinc 
            w9 = np.random.uniform( theta_here[9]*0.99  ,   theta_here[9]*1.01  ,   size=(nwalkers))    #pa [degrees]
            w10 = np.random.uniform( theta_here[10]*0.99  ,   theta_here[10]*1.01  ,   size=(nwalkers))    # Xoffset is in AU in minor axis direction + => towards NORTH WEST
            w11 = np.random.uniform( theta_here[11]*0.99  ,   theta_here[11]*1.01  ,   size=(nwalkers))    # Yoffset is in AU in major axis direction + => towards SOUTH WEST
            w12= np.random.uniform( theta_here[12]*0.999 ,   theta_here[12]*1.001  ,   size=(nwalkers))    #log normalizing factor

            p0 = np.dstack((w0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12))
            #############################################################

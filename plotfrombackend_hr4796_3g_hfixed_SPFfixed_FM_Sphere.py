...
chain4plot[:,:,3]=np.degrees(np.arccos(chain4plot[:,:,3]))
chain4plot[:,:,7]=np.exp(chain4plot[:,:,7])
...
labels=["R1[AU]","R2[AU]",r"$\beta$",r"$i[^{\circ}]$", r"$pa[^{\circ}]$",'dx[AU]', 'dy[AU]',"N[ADU]"]
...
fig.gca().annotate("SPHERE H2 band 3HG fixed, {0:,} iterations (192 walkers): {1:,} models".format(reader.iteration, reader.iteration*192),
...
names=["R1","R2","Beta", "inc", "PA",'dx', 'dy',\
    "N", "RA","Decl","a","ecc","Omega" ]

comments = ['AU, inner radius', 'AU, outer radius', 'radial power law', 'degree, inclination',\
...
theta_ml=[\
        ...
        np.cos(np.radians(MLval_mcmc_val_mcmc_err[3,0])),\
        MLval_mcmc_val_mcmc_err[4,0],\
        MLval_mcmc_val_mcmc_err[5,0],\
        MLval_mcmc_val_mcmc_err[6,0],\
        np.log(MLval_mcmc_val_mcmc_err[7,0])\
        ]
...
fig.suptitle('SPHERE H2 band (3g model fixed): Best Model and Residuals', fontsize=5/4.*caracsize, y = 0.985)

...
def integrand_dxdy_3g(xp,yp_dy2,yp2,zp,zp2,zpsi_dx,zpci,R1,R2,beta,a_r,g1,g1_2,g2,g2_2,g3,g3_2, alpha1, alpha2, ci, si,maxe,dx, dy,k):
...
def gen_disk_dxdy_3g(R1=74.42, R2=82.45, beta=1.0, aspect_ratio=0.1, g1=0.6, g2=-0.6 , g3=-0.1, alpha1=0.7, alpha2=0., inc=76.49, pa=30, distance=72.8, sampling=1, mask=None, dx=0, dy=0.):
    ...
    if len(np.shape(mask)) < 2:
        ...
        for i,yp in enumerate(y):
            for j,zp in enumerate(z):
                ...
                image[j,i]=  quad(integrand_dxdy_3g, -R2, R2, epsrel=0.5e-12,limit=75,args=(yy_dy2,y2,zp,z2,zpsi_dx,zpci,R1,R2,beta,a_r,g1,g1_2,g2,g2_2,g3,g3_2, alpha1, alpha2,ci,si,maxe,dx,dy,k))[0]
...
def call_gen_disk_3g(theta, wheremask2generatedisk):
    ...
    r2 = mt.exp(theta[1])
    beta = theta[2]
    g1 = 0.99997934 # theta[3]
    g2 = 0.05089959 # theta[4]
    g3 = 0.04771195 # theta[5]
    alpha1 = 0.99948175 # theta[6]
    alpha2 = -0.03397273 #theta[7]
    ...
    #generate the model
    model = norm*gen_disk_dxdy_3g(R1=r1,R2=r2, beta=beta, aspect_ratio=0.01, g1=g1, g2=g2,g3 =g3, alpha1=alpha1,alpha2=alpha2, inc=inc, pa=pa, distance=distance, dx=dx, dy=dy, mask = wheremask2generatedisk) #+ offset
...
def logl(theta):
    ...
    model=call_gen_disk_3g(theta, wheremask2generatedisk)
...
def logp(theta):
    ...
    if ((r2 > 82) and (r2 < 102)):
        prior_rout = 1./(1.+np.exp(200.*(r2-100)))
    else: return -np.inf
...
file_prefix = 'Sphere_hr4796_klipFM_3gfixed'
...
if first_time == 1:
    ...
    mask_disk_zeros =  make_disk_mask(newdim, 28., 76.,46.,120., xcen=xcen, ycen=ycen)
    ...
    mask_disk_zeros =  make_disk_mask(newdim, 28., 76.,46.,138., xcen=xcen, ycen=ycen)

    mask_speckle_region = np.ones((newdim,newdim))
...
if first_time == 1:
    ...
    parallelized.klip_dataset(dataset, numbasis=KLMODE, maxnumbasis=110, annuli=1, subsections=1, mode='ADI',
    ...
    reduced_data_nonflip = fits.getdata(klipdir + file_prefix + '_Measurewithparallelized-KLmodes-all.fits')[0] ### we take only the first KL mode
    reduced_data = np.flip(reduced_data_nonflip, axis = 0)
    del reduced_data_nonflip
    fits.writeto(klipdir + file_prefix+ '_Measurewithparallelized-KLmodes-all.fits', reduced_data, clobber='True')
...
if first_time == 1:
    ...
    # theta_here = (np.log(74.244177),np.log(100.00523484), 13.523917, 0.9058, 0.01,-0.1679, 0.3224, -0.01, np.cos(np.radians(76.76615)), 26.8840293,-3.11,1.49, np.log(138.38116))  
    theta_here = (np.log(74.2131),np.log(100.0141), 13.521147, np.cos(np.radians(76.76615)), 26.8840293,-3.11,1.49, np.log(138.38116))  

    #generate the model
    model_here =call_gen_disk_3g(theta_here, wheremask2generatedisk)
...
if first_time == 1:
    ...
    diskobj = DiskFM([size_datacube[0], newdim, newdim], KLMODE,dataset, model_here_convolved, annuli=1, subsections=1, 
                            basis_filename = klipdir + file_prefix+ '_KLbasis.pkl', save_basis = True, load_from_basis = False, numthreads =1)

    fm.klip_dataset(dataset, diskobj, numbasis=KLMODE,maxnumbasis = 110, annuli=1, subsections=1, mode='ADI',
...
if __name__ == '__main__':
    ...
    # with contextlib.closing(MPIPool()) as pool:
        ...
        if new_backend ==1:
            ...
            # w3 = np.random.uniform( theta_here[3]*0.999  ,   theta_here[3]  ,       size=(nwalkers))    #g1
            # w4 = np.random.uniform( theta_here[4]*0.999  ,   theta_here[4]*1.001  ,   size=(nwalkers))    #g2
            # w5 = np.random.uniform( theta_here[5]*0.999  ,   theta_here[5]*1.001  ,   size=(nwalkers))    #g3
            # w6 = np.random.uniform( theta_here[6]*0.999  ,   theta_here[6]  ,          size=(nwalkers))    #alpha1
            # w7 = np.random.uniform( theta_here[7]*0.999  ,   theta_here[7]*1.001  ,   size=(nwalkers))    #alpha2
            w3 = np.random.uniform( theta_here[3]*0.99  ,   theta_here[3]*1.01  ,   size=(nwalkers))    #cinc 
            w4 = np.random.uniform( theta_here[4]*0.99  ,   theta_here[4]*1.01  ,   size=(nwalkers))    #pa [degrees]
            w5 = np.random.uniform( theta_here[5]*0.99  ,   theta_here[5]*1.01  ,   size=(nwalkers))    # Xoffset is in AU in minor axis direction + => towards NORTH WEST
            w6 = np.random.uniform( theta_here[6]*0.99  ,   theta_here[6]*1.01  ,   size=(nwalkers))    # Yoffset is in AU in major axis direction + => towards SOUTH WEST
            w7= np.random.uniform( theta_here[7]*0.999 ,   theta_here[7]*1.001  ,   size=(nwalkers))    #log normalizing factor

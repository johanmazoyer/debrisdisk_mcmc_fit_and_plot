...
def gen_disk_dxdy_2g(dim, R1=74.42, R2=82.45, beta=1.0, aspect_ratio=0.1, g1=0.6, g2=-0.6, alpha=0.7, inc=76.49, pa=30, distance=72.8, sampling=1, mask=None, dx=0, dy=0., pixscale = pixscale):
...
def call_gen_disk_2g(theta, wheremask2generatedisk):
    ...
    #generate the model
    model = mt.exp(norm)*gen_disk_dxdy_2g(dimension, R1=r1,R2=r2, beta=beta, aspect_ratio=0.01, g1=g1, g2=g2,alpha=alpha, inc=inc, pa=pa, distance=distance, dx=dx, dy=dy, mask = wheremask2generatedisk, pixscale = pixscale) #+ offset
...
if first_time == 1:
    ...
    diskobj = DiskFM([len(filelist), 281, 281], KLMODE,dataset, model_here_convolved, annuli=1, subsections=1,
                            basis_filename = klipdir + file_prefix+ '_basis.pkl', save_basis = True, load_from_basis = False, numthreads =1)

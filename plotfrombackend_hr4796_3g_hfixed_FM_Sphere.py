...
chain4plot[:,:,8]=np.degrees(np.arccos(chain4plot[:,:,8]))
chain4plot[:,:,12]=np.exp(chain4plot[:,:,12])
...
chain4plot[:,:,6]=100*chain4plot[:,:,6]
chain4plot[:,:,7]=100*chain4plot[:,:,7]

f, axarr=plt.subplots(ndim, sharex=True, figsize = (6.4*quality_plot, 4.8*quality_plot))

labels=["R1[AU]","R2[AU]",r"$\beta$", "g1[%]","g2[%]","g3[%]",r"$\alpha$1[%]", r"$\alpha$2[%]",r"$i[^{\circ}]$", r"$pa[^{\circ}]$",'dx[AU]', 'dy[AU]',"N[ADU]"]
...
for i in range(ndim):
    mcmc = np.percentile(samples[:, i], [0.1/100., 50/100., 99.9/100.])
    print(labels[i]+' -/+3sigma : {0:.3f} -{1:.3f} +{2:.3f}'.format(mcmc[1], mcmc[1] -  mcmc[0], mcmc[2] -  mcmc[1]))
print("")
...
fig.gca().annotate("SPHERE H2 band 3HG, {0:,} iterations (192 walkers): {1:,} models".format(reader.iteration, reader.iteration*192),
                      xy=(0.55, 0.99), xycoords="figure fraction",
                      xytext=(-20, -10), textcoords="offset points",
                      ha="center", va="top", fontsize=42)
...
other_samples = np.zeros((samples.shape[0],5))

for j in range(samples.shape[0]):
    r1_here = samples[j,0]
    inc_here = samples[j,8]
    pa_here = samples[j,9]
    dx_here = samples[j,10]
    dy_here = samples[j,11]
...
MLval_mcmc_val_mcmc_err = np.zeros((ndim+5, 4))
names=["R1","R2","Beta", "g1","g2","g3","alph1","alph2", "inc", "PA",'dx', 'dy',\
    "N", "RA","Decl","a","ecc","Omega" ]

comments = ['AU, inner radius', 'AU, outer radius', 'radial power law', '%, 1st HG param', '%, 2nd HG param', '%, 3rd HG param','%, 2nd HG param','%, 2nd HG param', 'degree, inclination',\
    ...
    'deproj. (true) eccentricity (Kowalsky)', 'deg, longitude of the ascending node (Kowalsky)']
...
for i in range(5):
    ...
    percent = np.percentile(other_samples[:, i], [0.1, 50, 99.9])
...
theta_ml=[\
        ...
        1/100.*MLval_mcmc_val_mcmc_err[6,0],\
        1/100.*MLval_mcmc_val_mcmc_err[7,0],\
        np.cos(np.radians(MLval_mcmc_val_mcmc_err[8,0])),\
        MLval_mcmc_val_mcmc_err[9,0],\
        MLval_mcmc_val_mcmc_err[10,0],\
        MLval_mcmc_val_mcmc_err[11,0],\
        np.log(MLval_mcmc_val_mcmc_err[12,0])\
        ]
...
fig.suptitle('SPHERE H2 band (3g model): Best Model and Residuals', fontsize=5/4.*caracsize, y = 0.985)
